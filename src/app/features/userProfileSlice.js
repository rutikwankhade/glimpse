...
export const getUserProfile = createAsyncThunk(
  ...
  async ({ id }) => {
    ...
    try {
      const res = await axios.get(`https://glimpse-backend.onrender.com/api/user/${id}`);
...
export const addBookToLibrary = createAsyncThunk(
  ...
  async ({ bookId, status, title, cover }) => {
    ...
    try {
      const res = await axios.post('https://glimpse-backend.onrender.com/api/user/addbook', body, config);
...
export const followReader = createAsyncThunk(
  ...
  async (followReaderID) => {
    try {
      const response = await axios.patch(`https://glimpse-backend.onrender.com/api/user/follow/${followReaderID}`);
...
export const unfollowReader = createAsyncThunk(
  ...
  async (unfollowReaderID) => {
    try {
      const response = await axios.patch(`https://glimpse-backend.onrender.com/api/user/unfollow/v1/${unfollowReaderID}`);
      if (response.data.success) {
        return response.data;
      }
      return {};
...
export const getPostsByUserId = createAsyncThunk(
  ...
  async (userId) => {
    ...
    try {
      const res = await axios.get(`https://glimpse-backend.onrender.com/api/post/user/${userId}`);
